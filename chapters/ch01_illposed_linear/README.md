# Chapter 1 — Ill-posed Linear Inversion

## Overview

This directory corresponds to the Chapter 1 practice section on noise amplification in ill-posed linear inverse problems.

## Contents

`main.py` is a self-written teaching script for a blurred linear forward operator, direct pseudo-inverse reconstruction, and Tikhonov regularization.

## Source note

Self-written teaching script.

For the practice-section source catalogue, see [`../../third_party/SOURCE.md`](../../third_party/SOURCE.md). Manuscript-synchronized BibTeX entries for practice-section references and external repository entries are available in [`../../third_party/references.bib`](../../third_party/references.bib).
